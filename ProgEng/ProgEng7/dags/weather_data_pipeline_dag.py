    ...
    for index, row in df.iterrows():
        cursor.execute(
            ...
            (base, visibility, dt, timezone, id, name, cod, coord_lon, coord_lat,
             ...
             wind_speed, wind_deg, clouds_all, sys_type, sys_id, sys_country,
             sys_sunrise, sys_sunset, weather_id, weather_main,
             weather_description, weather_icon)
            VALUES (
                '{row["base"]}', {row["visibility"]}, {row["dt"]},
                {row["timezone"]}, {row["id"]}, '{row["name"]}', {row["cod"]},
                {row["coord.lon"]}, {row["coord.lat"]}, {row["main.temp"]},
                {row["main.feels_like"]}, {row["main.temp_min"]},
                {row["main.temp_max"]}, {row["main.pressure"]},
                {row["main.humidity"]}, {row["main.sea_level"]},
                {row["main.grnd_level"]}, {row["wind.speed"]}, {row["wind.deg"]},
                {row["clouds.all"]}, '{row["sys.type"]}', {row["sys.id"]},
                '{row["sys.country"]}', {row["sys.sunrise"]}, {row["sys.sunset"]},
                {row["weather.id"]}, '{row["weather.main"]}',
                '{row["weather.description"]}', '{row["weather.icon"]}'
            );
            """
        )
...
with DAG(
    ...
    description="A weather data pipeline from openweathermap for New Delhi",
